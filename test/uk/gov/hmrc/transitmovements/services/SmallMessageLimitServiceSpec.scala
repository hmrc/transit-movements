...
class SmallMessageLimitServiceSpec extends AnyFreeSpec with MockitoSugar with ScalaCheckDrivenPropertyChecks {
  ...
  "Small message limit " - {
    val service = new SmallMessageLimitServiceImpl(config)
    when(config.smallMessageSizeLimit).thenReturn(limit)

    "should return false when below the limit" in forAll(Gen.choose(1L, limit)) {
      size =>
        service.isLarge(size) mustBe false
    }

    "should return true when below the limit" in forAll(Gen.choose(limit + 1L, 5000000L)) {
      size =>
