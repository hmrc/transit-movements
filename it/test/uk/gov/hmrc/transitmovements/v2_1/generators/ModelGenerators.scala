...
import uk.gov.hmrc.transitmovements.v2_1.models.mongo.write
...
trait ModelGenerators extends BaseGenerators {
  ...
  implicit lazy val arbitraryMessage: Arbitrary[MongoMessage] =
    Arbitrary {
      ...
      } yield write.MongoMessage(id, received, generated, Some(messageType), triggerId, url, body, Some(size), Some(status))
    }
  ...
  implicit lazy val arbitraryMovement: Arbitrary[MongoMovement] =
    Arbitrary {
      ...
      } yield write.MongoMovement(id, movementType, eori, Some(eori), movementReferenceNumber, movementLRN, messageSender, created, updated, messages)
    }
